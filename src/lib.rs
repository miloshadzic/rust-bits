...
pub fn qsort<T: Ord + Copy>(arr: &mut [T]) {
    let lo = 0;
    let hi = arr.len();
    {
        qsort_sub(arr, lo, hi);
...
fn qsort_sub<T: Ord + Copy>(arr: &mut [T], lo: usize, hi: usize) {
    if lo >= hi {
        return;
    }
    let pivot = partition(arr, lo, hi);
    {
        qsort_sub(arr, lo, pivot);
        qsort_sub(arr, pivot + 1, hi);
    }
}
